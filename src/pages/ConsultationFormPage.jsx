...
const emptyForm = { date: '2026-07-20', type: '진로 탐색', purpose: '관심 직무 구체화 및 경험 계획 수립', currentConcern: '', rawMemo: '', guidance: '', programs: [], studentActions: '', counselorActions: '', nextCheckItems: '', nextDate: '2026-08-03' };
...
export default function ConsultationFormPage() {
  ...
  const [form, setForm] = useState(() => draftForm?.studentId === student.id ? draftForm.form : { ...emptyForm, currentConcern: student.concern, rawMemo: '개발 수업 경험을 되짚어 보니 문제를 정의하고 팀의 의견을 정리하는 과정에 흥미를 느꼈다고 함. 서비스 기획 직무를 직접 경험해 본 뒤 개발 직무와 비교하고 싶어 함.', studentActions: '관심 직무 비교표 작성 및 UX 서비스 기획 캠프 신청', counselorActions: '직무 비교표 양식과 캠프 상세 일정 전달', nextCheckItems: '직무 비교 결과와 캠프 신청 여부' });
  ...
  return <>
    ...
    <div className="consultation-layout">
      <section className="card consultation-form-card">
        ...
        <label>핵심 상담 메모 <span className="required">필수</span><textarea id="rawMemo" className="memo-area" rows="8" value={form.rawMemo} onChange={e => update('rawMemo', e.target.value)} placeholder="예: 개발 수업은 재미있었지만, 문제를 정의하고 사람들과 조율하는 역할에 더 흥미를 느낌..." /><small className="field-hint">메모가 구체적일수록 AI 초안이 더 정확하게 정리됩니다.</small>{error && <span className="field-error" role="alert">{error}</span>}</label>
      ...
      <aside className="consultation-aside">
        <section className="card student-context"><div className="context-head"><Avatar student={student} /><div><strong>{student.name}</strong><p>{student.department} · {student.grade}</p></div><StatusBadge status="inProgress" /></div><dl><div><dt>진로 목표</dt><dd>{student.goal}</dd></div><div><dt>최근 상담</dt><dd>{student.lastConsultation}</dd></div></dl></section>
