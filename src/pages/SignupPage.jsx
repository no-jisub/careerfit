...
import { validateStudentRegistrationInput } from '../utils/validation';
...
export default function SignupPage() {
  ...
  const { firebaseAuthEnabled, registerStudent, user, role } = useAuth();
  ...
  const submit = async event => {
    ...
    if (saving) return;
    const validated = validateStudentRegistrationInput(form);
    if (validated.error) { setError(validated.error); return; }
    setSaving(true);
    setError('');
    try {
      await registerStudent(validated.value);
  ...
  return <main className="auth-standalone">
    <section className="auth-card signup-card">
      <div className="auth-card-heading"><span className="brand-mark"><Icon name="target" size={24} /></span><div><span className="eyebrow">학생 회원가입</span><h1>상담을 시작할 정보를 알려주세요</h1><p>가입 후 이메일 인증과 상담사 배정을 완료하면 서비스를 이용할 수 있어요.</p></div></div>
      <form className="signup-form" onSubmit={submit}>
        ...
        <button className="button primary full" disabled={saving}>{saving ? '가입 처리 중...' : '학생 회원가입'}</button>
      </form>
