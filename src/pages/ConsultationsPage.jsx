...
export default function ConsultationsPage() {
  ...
  return <>
    <PageIntro eyebrow="상담 기록" title="상담의 흐름을 이어가세요" description="최근 작성된 상담 기록을 학생별로 확인할 수 있어요." action={<Link className="button primary" to="/students?select=consultation"><Icon name="plus" size={18} />상담 기록 작성</Link>} />
    ...
    <section className="card"><div className="list-toolbar"><div><h2>상담 기록 <span>{filtered.length}</span></h2><p>학생에게 공개되는지 함께 확인하세요.</p></div></div>{filtered.length ? <div className="consultation-index">{filtered.map(consultation => { const student = students.find(item => item.id === consultation.studentId); if (!student) return null; return <Link to={`/students/${student.id}`} key={consultation.id}><time>{consultation.date}</time><div><span className="tag">{consultation.type}</span><span className={`visibility-tag ${consultation.studentVisible === false ? 'private' : ''}`}>{consultation.studentVisible === false ? '학생 비공개' : '학생 공개'}</span><h2>{student.name} · {consultation.purpose}</h2><p>{consultation.summary}</p></div><Icon name="chevron" /></Link>; })}</div> : <EmptyState title="조건에 맞는 상담 기록이 없습니다" description="검색어나 필터를 바꾸어 보세요." />}</section>
  </>;
}
