...
export default function DashboardPage() {
  ...
  return <>
    ...
    <section className="summary-grid" aria-label="오늘의 상담 요약">
      <article className="summary-card blue"><span className="summary-icon"><Icon name="calendar" /></span><div><small>오늘 상담 예정</small><strong>{scheduled.length}<em>명</em></strong><p>{scheduled[0] ? <><b>다음</b> {getDayPeriod(scheduled[0].appointment)} {scheduled[0].appointment} {scheduled[0].name}</> : '예정된 상담이 없습니다'}</p></div></article>
    ...
    <div className="dashboard-grid">
      ...
      <section className="card span-2"><SectionHeader eyebrow="최근 활동" title="최근 상담 기록" action={<Link to="/consultations">전체 보기 <Icon name="chevron" size={16} /></Link>} />
        <div className="recent-list">{[...consultations].sort((a, b) => b.date.localeCompare(a.date)).slice(0, 3).map(c => { const student = studentById(c.studentId); return <Link to={`/students/${c.studentId}`} key={c.id}><Avatar student={student} size="small" /><div><strong>{student?.name}</strong><p>{c.purpose} · {c.summary}</p></div><time>{c.date.slice(5).replace('-', '.')}</time></Link>; })}</div>
      </section>
