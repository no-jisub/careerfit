...
const appointmentStatusOptions = [
  { value: 'all', label: '전체', description: '모든 상담 일정', icon: 'layers' },
  { value: 'pending', label: '승인 대기', description: '확인 필요', icon: 'clock' },
  { value: 'confirmed', label: '확정', description: '상담 예정', icon: 'calendar' },
  { value: 'completed', label: '완료', description: '상담 완료', icon: 'check' },
  { value: 'cancelled', label: '취소', description: '취소된 일정', icon: 'close' },
];
...
export default function AppointmentsPage() {
  ...
  const appointmentStatusCounts = appointments.reduce((counts, appointment) => {
    const groupedStatus = appointment.status === 'scheduled' ? 'confirmed' : appointment.status;
    counts[groupedStatus] = (counts[groupedStatus] || 0) + 1;
    return counts;
  }, {});
  const ordered = useMemo(() => appointments
    .filter(item => statusFilter === 'all' || (statusFilter === 'confirmed' ? ['confirmed', 'scheduled'].includes(item.status) : item.status === statusFilter))
    .filter(item => {
  ...
  return <>
    ...
    <section className="card appointment-filter-panel" aria-label="상담 일정 검색 및 필터">
      <StatusTabs
        className="appointment-status-tabs compact-status-tabs"
        label="상담 일정 상태"
        options={appointmentStatusOptions.map(option => ({ ...option, count: option.value === 'all' ? appointments.length : appointmentStatusCounts[option.value] || 0 }))}
        value={statusFilter}
        onChange={setStatusFilter}
      />
      <div className="appointment-filter-tools">
        <label className="search-field"><span className="sr-only">일정 검색</span><Icon name="search" size={19} /><input value={query} onChange={event => setQuery(event.target.value)} placeholder="학생 이름, 학번, 상담 유형 또는 장소 검색" /></label>
        {(query || statusFilter !== 'all') && <button className="button secondary small" onClick={() => { setQuery(''); setStatusFilter('all'); }}><Icon name="close" size={15} />필터 초기화</button>}
      </div>
    </section>
