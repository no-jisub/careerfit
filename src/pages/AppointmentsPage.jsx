...
export default function AppointmentsPage() {
  ...
  const ordered = useMemo(() => [...appointments].sort((a, b) => `${a.date}T${a.time}`.localeCompare(`${b.date}T${b.time}`)), [appointments]);
  ...
  const save = async event => {
    ...
    if (!student) return;
    const conflict = appointments.some(item => item.id !== editingId && item.status === 'scheduled' && item.counselorUid === student.counselorUid && item.date === form.date && item.time === form.time);
    if (conflict) {
  ...
  return <>
    ...
    <section className="card appointment-list-card">
      <div className="list-toolbar"><div><h2>전체 일정 <span>{ordered.length}</span></h2><p>날짜와 시간순으로 표시됩니다.</p></div></div>
