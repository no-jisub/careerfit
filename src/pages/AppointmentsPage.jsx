...
import { activeAppointmentStatuses, hasCounselorAppointmentConflict } from '../utils/appointments';
import { useAuth } from '../auth/AuthContext';
import { validateAvailabilityInput } from '../utils/validation';

const emptyForm = () => ({ studentId: '', date: toDateKey(), time: '10:00', type: '진로 상담', location: '대학일자리플러스센터 상담실 2', preparation: '' });
const emptyAvailabilityForm = () => ({ date: toDateKey(), time: '10:00', duration: 50, location: '대학일자리플러스센터 상담실 2' });
...
export default function AppointmentsPage() {
  ...
  const save = async event => {
    ...
    const conflict = hasCounselorAppointmentConflict(appointments, students, { ...form, counselorUid: student.counselorUid }, editingId);
    if (conflict) {
  ...
  const updateStatus = async (appointment, status) => {
    const now = new Date().toISOString();
    const updated = { ...appointment, status, updatedAt: now, ...(status === 'cancelled' ? { cancelledAt: now } : {}), ...(status === 'confirmed' ? { confirmedAt: now } : {}), ...(status === 'completed' ? { completedAt: now } : {}) };
  ...
  const saveAvailability = async event => {
    ...
    if (myAvailability.some(item => item.status !== 'closed' && item.date === validated.value.date && item.time === validated.value.time)) {
      setAvailabilityError('같은 날짜와 시간에 이미 상담 가능 시간이 등록되어 있습니다.');
      return;
    }
    const now = new Date().toISOString();
    const availability = {
    ...
    try {
      ...
      setCounselorAvailability(items => [...items, availability]);
  ...
  return <>
    ...
    <section className="card appointment-list-card">
      ...
      {ordered.length ? <div className="appointment-list">{ordered.map(item => { const student = students.find(candidate => candidate.id === item.studentId); const active = activeAppointmentStatuses.includes(item.status); return <article key={item.id} className={item.status}><time><strong>{item.date}</strong><span>{item.time}</span></time><div><Link to={`/students/${item.studentId}`}>{student?.name || '학생'}</Link><p>{item.type} · {item.location}</p>{item.requestedBy === 'student' ? <details className="appointment-request-details"><summary>학생 사전 상담 내용 보기</summary><dl><div><dt>상담 주제</dt><dd>{item.subject}</dd></div><div><dt>전달 내용</dt><dd>{item.requestMessage}</dd></div>{item.preferredOutcome && <div><dt>원하는 결과</dt><dd>{item.preferredOutcome}</dd></div>}</dl></details> : <small>{item.preparation ? `준비사항: ${item.preparation}` : '별도 준비사항 없음'}</small>}</div><span className={`appointment-status ${item.status}`}>{appointmentStatusLabels[item.status] || item.status}</span><div className="appointment-actions">{active && <><button className="button secondary small" onClick={() => openEdit(item)}>변경</button>{item.status === 'pending' && <button className="button primary small" onClick={() => updateStatus(item, 'confirmed')}>확정</button>}{['confirmed', 'scheduled'].includes(item.status) && <button className="button primary small" onClick={() => updateStatus(item, 'completed')}>완료</button>}<button className="text-button danger" onClick={() => updateStatus(item, 'cancelled')}>취소</button></>}</div></article>; })}</div> : <EmptyState icon="calendar" title="등록된 상담 일정이 없습니다" description="상담 예약 버튼으로 첫 일정을 등록해 주세요." />}
    </section>
