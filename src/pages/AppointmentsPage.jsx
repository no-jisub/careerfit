...
export default function AppointmentsPage() {
  ...
  return <>
    ...
    <section className="card appointment-list-card">
      ...
      {ordered.length ? <div className="appointment-list">{ordered.map(item => { const student = students.find(candidate => candidate.id === item.studentId); const active = activeAppointmentStatuses.includes(item.status); return <article key={item.id} className={item.status}><time><strong>{item.date}</strong><span>{item.time}</span></time><div><Link to={`/students/${item.studentId}`}>{student?.name || '학생'}</Link><p>{item.type} · {item.location}</p><small>{item.preparation ? `준비사항: ${item.preparation}` : '별도 준비사항 없음'}</small></div><span className={`appointment-status ${item.status}`}>{appointmentStatusLabels[item.status] || item.status}</span><div className="appointment-actions">{active && <><button className="button secondary small" onClick={() => openEdit(item)}>변경</button>{item.status === 'pending' && <button className="button primary small" onClick={() => updateStatus(item, 'confirmed')}>확정</button>}{['confirmed', 'scheduled'].includes(item.status) && <button className="button primary small" onClick={() => updateStatus(item, 'completed')}>완료</button>}<button className="text-button danger" onClick={() => updateStatus(item, 'cancelled')}>취소</button></>}</div></article>; })}</div> : <EmptyState icon="calendar" title="등록된 상담 일정이 없습니다" description="상담 예약 버튼으로 첫 일정을 등록해 주세요." />}
    </section>
