...
export default function StudentMyPage() {
  ...
  return <div className="student-portal"><header><div className="brand"><span className="brand-mark"><Icon name="target" size={22} /></span><span>커리어<span>핏</span></span></div><div><button className="icon-button" aria-label="알림"><Icon name="bell" /></button><strong>{student.name}</strong><button className="text-button" onClick={logout}>로그아웃</button></div></header><main>
    ...
    <section className="student-section card student-record-management"><div className="section-header"><div><span className="eyebrow">나의 상담 여정</span><h2>상담 기록 타임라인</h2><p>지금까지 상담한 흐름을 날짜순으로 살펴보고, 각 기록을 눌러 공개된 내용을 확인할 수 있어요.</p></div><span className="student-record-count">총 {visibleConsultations.length}건</span></div>{visibleConsultations.length ? <div className="student-consultation-timeline">{visibleConsultations.map(item => { const request = recordDeletionRequests.find(candidate => candidate.consultationId === item.id); const expanded = expandedConsultationId === item.id; return <article key={item.id} className={expanded ? 'open' : ''}><span className="student-timeline-dot" aria-hidden="true" /><button className="student-record-toggle" aria-expanded={expanded} aria-controls={`student-consultation-${item.id}`} onClick={() => setExpandedConsultationId(expanded ? '' : item.id)}><div className="student-record-info"><time className="student-record-date" dateTime={item.date}>{item.date}</time><div><strong>{item.purpose}</strong><span>{item.type} · {item.counselor || student.counselor || '담당 상담사'}</span></div></div><Icon name="chevron" size={18} /></button>{expanded && <div className="student-timeline-body" id={`student-consultation-${item.id}`}><div className="published-summary-fields">{item.summary && <div><span>상담 요약</span><p>{item.summary}</p></div>}{item.strengths && <div><span>나의 강점</span><p>{item.strengths}</p></div>}{item.concern && <div><span>개선 또는 고민 사항</span><p>{item.concern}</p></div>}{item.programs?.length > 0 && <div><span>추천 프로그램</span><p>{item.programs.join(', ')}</p></div>}{item.studentActions && <div><span>후속 조치</span><p>{item.studentActions}</p></div>}{item.nextCheckItems && <div><span>다음 상담 계획</span><p>{item.nextCheckItems}</p></div>}</div><div className="student-timeline-footer"><small>상담일 {item.date} · {item.counselor || student.counselor || '담당 상담사'} 상담사</small>{request ? <span className={`request-status ${request.status}`}>{request.status === 'pending' ? '삭제 검토 중' : request.status === 'approved' ? '삭제 승인' : '삭제 반려'}</span> : <button className="text-button danger student-record-delete" onClick={() => requestRecordDeletion(item)}>삭제 요청</button>}</div></div>}</article>; })}</div> : <EmptyState title="공개된 상담 기록이 없습니다" description="상담사가 기록을 공개하면 이곳에서 확인할 수 있어요." />}</section>
