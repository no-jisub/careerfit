...
export default function StudentMyPage() {
  ...
  const withdrawAccount = async () => {
    if (withdrawalWorking || !window.confirm('회원 탈퇴를 요청할까요? 계정은 즉시 중지되고 30일 동안 복구할 수 있습니다.')) return;
    ...
    try {
      if (user) {
        ...
        const updated = { ...student, accountStatus: 'withdrawalPending', withdrawalRequestedAt: now.toISOString(), deletionScheduledAt: deletionDate.toISOString(), updatedAt: now.toISOString() };
        setStudents(items => items.map(item => item.id === student.id ? updated : item));
        notify(`데모 탈퇴 요청을 저장했습니다. 삭제 예정일은 ${deletionDate.toISOString().slice(0, 10)}입니다.`);
      }
    } catch { notify('탈퇴 요청을 처리하지 못했습니다. 잠시 후 다시 시도해 주세요.'); }
    finally { setWithdrawalWorking(false); }
  };
  return <div className="student-portal"><header><div className="brand"><span className="brand-mark"><Icon name="target" size={22} /></span><span>커리어<span>핏</span></span></div><div><button className="icon-button" aria-label="알림"><Icon name="bell" /></button><strong>{student.name}</strong><button className="text-button" onClick={logout}>로그아웃</button></div></header><main>
    ...
    <section className="student-section card student-account-actions"><div><span className="eyebrow">계정 관리</span><h2>회원 탈퇴</h2><p>탈퇴하면 즉시 이용이 중지되며 30일 동안 상담사를 통해 복구할 수 있습니다.</p></div><button className="button secondary danger" disabled={withdrawalWorking || student.accountStatus === 'withdrawalPending'} onClick={withdrawAccount}>{student.accountStatus === 'withdrawalPending' ? `삭제 예정 ${student.deletionScheduledAt?.slice(0, 10)}` : withdrawalWorking ? '처리 중...' : '회원 탈퇴 요청'}</button></section>
