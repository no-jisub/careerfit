...
export default function StudentMyPage() {
  ...
  const [qaMessages, setQaMessages] = useState([{ role: 'assistant', text: '상담 전에 필요한 것들이나 가볍게 궁금한 점을 물어보세요. 준비물, 고민 정리, 질문 만드는 방법을 바로 안내해 드릴게요.' }]);
  useEffect(() => {
  ...
  const quickQuestions = ['상담 전에 뭘 준비해?', '고민은 어떻게 정리해?', '질문은 어떻게 만들까?', '상담 후에는 뭘 하면 돼?'];
  ...
  const createQaAnswer = question => {
    ...
    if (normalized.includes('상담전') || normalized.includes('준비') || normalized.includes('필요') || normalized.includes('가져')) {
      return `상담 전에는 거창한 준비보다 지금 상태를 짧게 정리해 오는 게 가장 좋아요.\n1. 현재 진로 목표: ${student.goal}\n2. 가장 큰 고민: ${student.concern}\n3. 상담에서 꼭 묻고 싶은 질문 1~2개\n4. 최근에 해본 활동이나 관심 있는 비교과 프로그램`;
    }
    if (normalized.includes('고민') || normalized.includes('정리') || normalized.includes('막막') || normalized.includes('불안')) {
      return `고민은 한 문장으로 정리하면 상담이 훨씬 쉬워져요.\n예: "관심 분야는 있는데 어떤 직무와 연결되는지 모르겠어요."\n지금 등록된 고민은 "${student.concern}"입니다. 여기에 왜 고민되는지, 이미 해본 것, 결정하기 어려운 이유를 한 줄씩 붙여 보세요.`;
    }
    if (normalized.includes('질문') || normalized.includes('물어') || normalized.includes('궁금')) {
      return `상담 질문은 "상황 + 궁금한 점" 형태로 만들면 좋아요.\n예: "포트폴리오를 만들고 있는데 어떤 경험을 먼저 넣어야 할까요?"\n예: "공공기관을 희망하는데 지금 신청하면 좋은 비교과가 있을까요?"\n질문이 길지 않아도 괜찮고, 아직 정리가 안 된 상태 그대로 가져와도 됩니다.`;
    }
    if (normalized.includes('상담후') || normalized.includes('다음') || normalized.includes('끝나고') || normalized.includes('이후')) {
      if (!studentTasks.length) return '상담 후에는 상담사가 정리한 다음 행동이 생기면 이 화면에 표시됩니다. 아직 등록된 해야 할 일이 없으면 상담 내용을 간단히 메모해 두는 것부터 시작해 보세요.';
      return '상담 후에는 아래 "이번 상담 후 해야 할 일"에서 다음 행동을 확인하세요. 완료한 항목은 체크하면 흐리게 남아서 진행 이력으로 계속 볼 수 있어요.';
    }
    ...
    return '상담 전에 필요한 것들을 가볍게 물어볼 수 있어요. 예를 들어 "상담 전에 뭘 준비해?", "고민은 어떻게 정리해?", "질문은 어떻게 만들까?", "상담 후에는 뭘 하면 돼?"처럼 물어보세요.';
  };
