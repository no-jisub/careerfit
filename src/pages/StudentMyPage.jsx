...
export default function StudentMyPage() {
  ...
  const [qaMessages, setQaMessages] = useState([{ role: 'assistant', text: '담당 상담사가 바로 답하기 어려울 때, 진로 고민과 다음 준비 방향을 함께 정리해 드릴게요.' }]);
  useEffect(() => {
  ...
  const quickQuestions = ['진로 방향이 고민돼요', '면접 준비 어떻게 할까요?', '포트폴리오를 봐주세요', '공공기관 준비가 궁금해요'];
  ...
  const createQaAnswer = question => {
    ...
    if (normalized.includes('프로그램') || normalized.includes('비교과') || normalized.includes('추천')) {
      return `지금 상황에서는 아래 비교과를 먼저 살펴보면 좋아요.\n${recommendedPrograms.map(program => `${program.name}: ${program.reason}`).join('\n')}\n신청 전에는 일정과 참여 방식이 내 학업 일정과 맞는지도 확인하세요.`;
    }
    if (normalized.includes('해야') || normalized.includes('할일') || normalized.includes('todo') || normalized.includes('뭐부터')) {
      ...
      return `상담사가 없을 때는 기한이 가까운 일부터 처리하면 흔들림이 줄어요.\n${pendingStudentTasks.map((task, index) => `${index + 1}. ${task.content} (${task.dueDate}까지)`).join('\n')}`;
    }
    if (normalized.includes('상담') || normalized.includes('일정') || normalized.includes('준비')) {
      if (!nextAppointment) return '예정된 상담 일정은 아직 없습니다. 그동안 진로 목표, 관심 직무, 궁금한 점을 짧게 메모해 두면 다음 상담 때 바로 이어갈 수 있어요.';
      return `다음 상담은 ${nextAppointment.date} ${getDayPeriod(nextAppointment.time)} ${nextAppointment.time}, ${nextAppointment.location}에서 진행됩니다. 준비할 내용은 ${nextAppointment.preparation || '별도 없음'}입니다. 그 전에 진로 고민과 질문 2~3개를 정리해 두면 좋아요.`;
    }
    ...
    return `상담사가 부재중일 때 임시 상담 기준으로 정리해볼게요. 현재 목표는 "${student.goal}"이고 남은 할 일은 ${pendingStudentTasks.length}개입니다. 지금은 1) 관심 직무를 좁히기, 2) 관련 경험을 만들기, 3) 다음 상담에서 확인할 질문을 적어두기 순서가 좋습니다. "면접", "포트폴리오", "공공기관", "진로 고민"처럼 물어보면 더 구체적으로 도와드릴게요.`;
  };
  ...
  return <div className="student-portal"><header><div className="brand"><span className="brand-mark"><Icon name="target" size={22} /></span><span>커리어<span>핏</span></span></div><div><button className="icon-button" aria-label="알림"><Icon name="bell" /></button><strong>{student.name}</strong><button className="text-button" onClick={logout}>로그아웃</button></div></header><main>
    ...
    <section className="card student-ai-qa"><div className="section-header"><div><span className="eyebrow">AI 상담</span><h2>상담사 부재 시 진로 상담</h2><p>상담사가 바로 답하기 어려울 때 진로 고민, 면접, 포트폴리오, 비교과 준비 방향을 먼저 정리합니다.</p></div></div><div className="qa-thread" aria-live="polite">{qaMessages.map((message, index) => <div className={`qa-message ${message.role}`} key={`${message.role}-${index}`}><strong>{message.role === 'assistant' ? 'AI 상담' : student.name}</strong><p>{message.text}</p></div>)}</div><div className="qa-quick">{quickQuestions.map(question => <button type="button" key={question} onClick={() => askQa(question)}>{question}</button>)}</div><form className="qa-form" onSubmit={event => { event.preventDefault(); askQa(qaQuestion); }}><label className="sr-only" htmlFor="student-ai-question">AI 상담에 질문하기</label><input id="student-ai-question" value={qaQuestion} onChange={event => setQaQuestion(event.target.value)} placeholder="예: 진로가 막막한데 무엇부터 정리하면 좋을까?" /><button className="button primary">상담하기</button></form></section>
