...
export default function StudentsPage() {
  ...
  const filtered = useMemo(() => students.filter(s => {
    ...
    return (!q || [s.name, s.studentNo, s.department, ...(s.interests || [])].some(v => v.toLowerCase().includes(q))) && (grade === 'all' || s.grade === grade) && (status === 'all' || s.status === status) && (hasTask === 'all' || (hasTask === 'yes' ? pendingCount > 0 : pendingCount === 0)) && (tag === 'all' || s.interests?.includes(tag));
  }), [students, followUps, query, grade, status, hasTask, tag]);
  ...
  return <>
    ...
    <section className="filter-card" aria-label="학생 검색 및 필터">
      ...
      <label><span>상담 후 할 일</span><select value={hasTask} onChange={e => setHasTask(e.target.value)}><option value="all">전체</option><option value="yes">미완료 있음</option><option value="no">미완료 없음</option></select></label>
    </section>
    <section className="tag-filter-card" aria-label="학생 태그 필터">
      <span>학생 태그</span>
      <button className={tag === 'all' ? 'active' : ''} onClick={() => setTag('all')}>전체</button>
      {studentTagOptions.map(option => <button key={option} className={tag === option ? 'active' : ''} onClick={() => setTag(option)}>{option}</button>)}
    </section>
    <section className="card student-list-card" aria-labelledby="student-list-title">
      <div className="list-toolbar"><div><h2 id="student-list-title">전체 학생 <span aria-live="polite">{filtered.length}</span></h2><p>최근 상담일 순으로 표시됩니다.</p></div><button className="text-button" onClick={() => { setQuery(''); setGrade('all'); setStatus('all'); setHasTask('all'); setTag('all'); }}>필터 초기화</button></div>
