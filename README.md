...
- 데모 상담사 박지현과 데모 학생 김하늘(`s1`)은 예약, 상담 공개 요약, 할 일,
  프로그램 추천·응답, 프로필 변경과 역할별 알림을 동일한 브라우저 저장소에서 공유
- 데모 역할은 탭별 `sessionStorage`에 저장되어 상담사·학생 화면을 두 탭에 동시에
  열 수 있고, 상호작용 데이터는 `storage` 이벤트로 즉시 동기화
