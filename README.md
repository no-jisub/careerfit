...
| 현재 운영 모드 | 데모 모드 |
| Firebase Authentication | 코드 준비 완료, 운영 빌드에서는 비활성화 |
| Cloud Firestore 동기화 | 코드·규칙 준비 완료, 운영 빌드에서는 비활성화 |
| 외부 AI API | 연동하지 않음 |

### 반드시 알아둘 현재 상태

- Firebase Hosting 배포와 GitHub Actions 자동 배포는 정상적으로 연결되어 있습니다.
- 현재 GitHub Actions는 `VITE_FIREBASE_AUTH_ENABLED=false`, `VITE_FIRESTORE_SYNC_ENABLED=false`로 빌드합니다.
- 인증 기능이 꺼져 있으면 코드에서 자동으로 데모 모드를 사용합니다. 따라서 `VITE_DEMO_MODE_ENABLED=false`여도 현재 운영 사이트에서는 상담 담당자/학생 역할을 선택해 데모에 들어갈 수 있습니다.
- 데모 데이터와 사용자가 변경한 내용은 브라우저 `localStorage`에만 저장됩니다.
...
| CI/CD | GitHub Actions | `main` 자동 빌드·배포 |
| CI 인증 | Google Workload Identity Federation | 서비스 계정 JSON 키 없는 단기 OIDC 인증 |

현재 `package.json`에는 테스트 프레임워크, 린터와 포매터 스크립트가 없습니다. 기능을 확장할 때 Vitest/React Testing Library와 ESLint 도입을 우선 검토하세요.
...
2. `AuthContext.jsx`가 Firebase 로그인 또는 데모 역할을 결정합니다.
3. `App.jsx`가 역할별 라우트를 보호하고 전역 데이터를 관리합니다.
4. 데모 모드에서는 `src/data/`의 초기 데이터와 `localStorage`를 사용합니다.
5. 실제 인증·동기화가 모두 활성화되면 `firebaseDataService.js`가 역할별 Firestore 쿼리를 구독합니다.
...
현재 자동화된 테스트 명령은 없습니다. 최소 검증은 아래와 같습니다.
...
- 상담 담당자는 자신에게 배정된 학생과 자신이 소유한 기록만 접근
- 학생은 자신의 공개 상담 기록과 자신에게 배정된 후속 조치만 읽기
...
동시 배포는 `firebase-hosting-live` concurrency group으로 묶이며, 새 `main` 배포가 시작되면 진행 중인 이전 배포를 취소하고 최신 커밋을 배포합니다.

## 실제 Auth/Firestore를 활성화하는 순서

현재 데모 운영 상태에서 다음 순서를 지키세요.

1. Firebase Console에서 Authentication 이메일/비밀번호 로그인을 활성화합니다.
2. Firestore 데이터베이스를 생성하고 리전을 확정합니다.
3. Emulator에서 `firestore.rules`와 실제 데이터 구조를 검증합니다.
4. 관리자 방식으로 사용자와 `users/{uid}` 역할 문서를 생성합니다.
5. 학생 문서 `uid`, 상담 담당자 문서 `counselorUid`와 각 기록의 UID 필드를 실제 Auth UID에 연결합니다.
6. 필요한 Firestore 인덱스를 추가합니다.
7. `npm run firebase:deploy:rules`로 규칙과 인덱스를 먼저 배포합니다.
8. GitHub Actions의 `VITE_FIREBASE_AUTH_ENABLED`를 `true`로 변경합니다.
9. 인증 흐름을 확인한 후 `VITE_FIRESTORE_SYNC_ENABLED`를 `true`로 변경합니다.
10. 데모 역할 선택을 운영에서 막으려면 `VITE_DEMO_MODE_ENABLED=false`를 유지합니다.

Auth와 Firestore를 한 번에 무검증 활성화하지 마세요. 실제 데이터가 없거나 UID 필드가 맞지 않으면 로그인 후 빈 화면 또는 권한 오류가 발생할 수 있습니다.
...
1. 운영 사이트는 아직 실제 Firebase Auth/Firestore를 사용하지 않습니다.
2. 상담 초안 생성은 실제 AI가 아니라 규칙 기반 로컬 함수입니다.
3. 프로그램 정보는 `src/data/programs.js`의 정적 데이터입니다.
4. 자동 테스트, 린트와 코드 포맷 검사가 없습니다.
5. Firebase App Check와 Analytics는 현재 초기화하지 않습니다.
6. Firestore 인덱스는 아직 정의되어 있지 않습니다.
7. 실제 대학 포털, 예약, 프로그램 신청과 알림 시스템은 연동되지 않았습니다.
